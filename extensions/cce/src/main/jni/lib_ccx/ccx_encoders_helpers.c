...
void call_function_if_match(unsigned char *line, struct word_list *list, void (*modification)(size_t, unsigned char *))
{
	...
	unsigned char *line_token = (unsigned char*) strdup((char*) line);
	unsigned char *c = (unsigned char*) strtok((char*)line_token, (char*)delim);
	...
	if (c != NULL)
	{
		...
		} while ((c = (unsigned char*)strtok(NULL, (char*) delim)) != NULL);
	}
	free(line_token);
}
