	...
	for (;;)
	{
		...
		else
		{
			...
			if (!ccx_options.live_stream)
			{
				...
				ctx->inputsize = ctx->total_inputsize;
				if (!ccx_options.binary_concat)
		...
		while ((!eof || ccx_options.live_stream) && bytes)
		{
			...
			if (ready == 0) // We really need to read more
			{
				...
				if (ccx_options.input_source == CCX_DS_FILE || ccx_options.input_source == CCX_DS_STDIN) {
				    ...
				    while(shared_byte_array_size < 1024 * 128) {
						LOGI("Waiting more data: Current size is %d", shared_byte_array_size);
                        if(log_fp) fprintf(log_fp, "Waiting more data: Current size is %d\n", shared_byte_array_size);
				        sleep(1);
				    }
