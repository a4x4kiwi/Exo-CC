...
int write_stringz_as_ssa(char *string, struct encoder_ctx *context, LLONG ms_start, LLONG ms_end)
{
	...
	while (pos_r < len)
	{
		if (string[pos_r] == '\r' && string[pos_r + 1] == '\n')
		{
			unescaped[pos_w++] = '\\';
			unescaped[pos_w] = 'N';
			pos_r += 2;
	...
	unsigned char *tmp = malloc(len + context->encoded_crlf_length + 30);
	int cur = 0;
	...
	memcpy(tmp + cur, context->encoded_crlf, context->encoded_crlf_length);
	cur += context->encoded_crlf_length;
	tmp[cur] = 0;
